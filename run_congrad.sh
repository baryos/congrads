...
roi='/nafs/kcross/PROJECTS/Cueing_Noninvasive_Auditory/mri/data/ROIs/harvardoxford-subcortical_prob_RightPutamen_MNI152_T1_2mm_bin.nii.gz'
...
for input_file in $files; do
  ...
  out_dir="/nafs/kcross/PROJECTS/Cueing_Noninvasive_Auditory/mri/data/preprocess/M2_results/$cm_subdirectory/$filename_noext"
  mkdir -p "$out_dir"
  ./congrads -i "$input_file" -r "$roi" -m "$mask" -o "$out_dir" -n 3 -s -p -f 2 2>> "$error_log"
done